...
// The number of elements in index_pairs must be at least 2*(n_rows+n_cols-1)
size_t strided_mask_to_index_pairs(const strided_mask_t* mask, size_t* index_pairs) {
    size_t index = 0;
    for (size_t row = 0; row < mask->n_rows; row++) {
        for (size_t col = mask->start_cols[row]; col <= mask->end_cols[row]; col++) {
            index_pairs[index++] = row;
            index_pairs[index++] = col;
        }
    }
    // Is this needed?
...
void strided_mask_expand(const strided_mask_t* mask_in, const size_t row_parity, const size_t col_parity, const size_t radius, strided_mask_t* mask_out) {
    ...
    for (int row = 0; row < (int) n_rows_final; row++) {
        ...
        mask_out->end_cols[row]   = (size_t) MIN((int) (2 *   mask_in->end_cols[next_row] + 1) + (int) radius + (int) col_parity, (int) n_cols_final-1);
    }
}
